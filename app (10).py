...
def verify_records():
    ...
    dns_results = {}
    records_to_update = []   # ho_ip corrections to save back
    ...
    for rec in valid_targets:
        ...
        if err:
            ...
            # current_ip matches expected source — ready to change
            status     = "ready"
            can_change = True
        else:
            # current_ip matches neither source nor target
            # For DR direction: current is already DR ip → treat as ready, update ho_ip
            if direction == "DR" and current_ip == rec.get("dr_ip"):
                # DNS is already at DR — mark done
                status     = "already_done"
                issues.append(f"Already pointing to DR IP {current_ip}")
                can_change = False
            elif direction == "HO" and current_ip == rec.get("dr_ip"):
                # Rolling back to HO, currently at DR — this is the normal rollback state
                # Auto-correct stored ho_ip using live DNS if ho_ip is empty/missing
                if not rec.get("ho_ip"):
                    rec["ho_ip"] = current_ip
                    records_to_update.append(rec)
                    issues.append(f"HO IP was empty — set to live DNS IP {current_ip}")
                    target_ip = current_ip  # can't rollback without ho_ip
                    status    = "invalid"
                    can_change = False
                else:
                    status     = "warning"
                    issues.append(
                        f"Current IP {current_ip} doesn't match expected DR IP {source_ip}"
                    )
                    can_change = True
            else:
                # Truly unexpected IP
    ...
    # Save back any auto-corrected ho_ip / dr_ip values
    if records_to_update:
        all_stored = load_records()
        updated_ids = {str(r["id"]): r for r in records_to_update}
        for i, stored_rec in enumerate(all_stored):
            rid = str(stored_rec.get("id"))
            if rid in updated_ids:
                all_stored[i]["ho_ip"] = updated_ids[rid]["ho_ip"]
                all_stored[i]["dr_ip"] = updated_ids[rid]["dr_ip"]
        save_records(all_stored)
