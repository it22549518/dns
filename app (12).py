...
BIND_HOST    = "127.0.0.1"
BIND_PORT    = 5000
PARALLEL_WORKERS = 5     # simultaneous WinRM batch sessions (don't overload DNS server)
BATCH_SIZE = 50          # records per single PowerShell batch call
...
def batch_dns_get_records(records):
    """
    Fetch IPs for a batch in ONE WinRM call.
    Uses idx-based keys (0,1,2...) to avoid float ID precision issues.
    Output format per record: RESULT|idx|OK|ip  or  RESULT|idx|ERR|msg
    ...
    for idx, r in enumerate(records):
        idx_to_rid[str(idx)] = str(r["id"])
        zone = str(r.get("zone","")).replace("'","").replace('"',"")
        name = str(r.get("record_name","")).replace("'","").replace('"',"")
        stmts.append(
            f"try{{"
            f"$_ip=(Get-DnsServerResourceRecord -ZoneName '{zone}'"
            f" -Name '{name}' -RRType A -EA Stop"
            f"|Select-Object -First 1).RecordData.IPv4Address.IPAddressToString;"
            f'Write-Host "RESULT|{idx}|OK|$_ip"'
            f"}}catch{{"
            f'Write-Host "RESULT|{idx}|ERR|$($_.Exception.Message)"'
            f"}}"
        )
...
"""
    out, _ = run_ps(ps, timeout=max(90, len(records) * 3))

    results = {}
    for line in out.splitlines():
        # Strip WinRM hostname prefix e.g. "DNSSERVER01 : RESULT|..."
        clean = line.strip()
        if "RESULT|" in clean:
            clean = clean[clean.index("RESULT|"):]
            parts = clean.split("|", 3)
            if len(parts) == 4:
                _, idx_s, status, value = parts
                rid = idx_to_rid.get(idx_s.strip())
                if rid is None:
                    continue
                if status.strip() == "OK":
                    results[rid] = (value.strip(), None)
                else:
                    err = value.strip()
                    not_found = any(x in err.lower() for x in
                        ["not found","does not exist","no records","objectnotfound"])
                    results[rid] = (None, ("NOT_FOUND:" if not_found else "") + err)
...
def batch_dns_set_records(records, direction):
    """
    Set IPs for a batch of records in a SINGLE WinRM session.
    Far more reliable than one session per record — avoids connection overload.
    Returns: list of result dicts (same shape as _change_one output).
    """
    if not records:
        return []

    # Build all SET statements in one scriptblock, indexed to avoid float ID issues
    idx_to_rec = {str(i): r for i, r in enumerate(records)}
    stmts = ""
    for idx, r in enumerate(records):
        zone      = str(r.get("zone","")).replace("'","").replace('"',"")
        name      = str(r.get("record_name","")).replace("'","").replace('"',"")
        target_ip = r["dr_ip"] if direction == "DR" else r["ho_ip"]
        target_ip = str(target_ip).replace("'","").replace('"',"")
        stmts += (
            f"try{{"
            f"$ex=(Get-DnsServerResourceRecord -ZoneName '{zone}' -Name '{name}' -RRType A -EA Stop|Select -First 1);"
            f"$up=$ex.Clone();"
            f"$up.RecordData.IPv4Address=[System.Net.IPAddress]::Parse('{target_ip}');"
            f"Set-DnsServerResourceRecord -ZoneName '{zone}' -OldInputObject $ex -NewInputObject $up -EA Stop;"
            f"Write-Output 'OK|{idx}'"
            f"}}catch{{Write-Output 'ERR|{idx}|'+$_.Exception.Message}}\n"
        )
...
"""
    timeout = max(120, len(records) * 8)
    out, _ = run_ps(ps, timeout=timeout)

    # Parse results
    parsed = {}
    for line in out.splitlines():
        line = line.strip()
        # WinRM sometimes prefixes lines with "hostname : " — strip it
        if " : " in line:
            line = line.split(" : ", 1)[-1].strip()
        if line.startswith("OK|"):
            parts = line.split("|", 2)
            if len(parts) >= 2:
                parsed[parts[1].strip()] = (True, "Updated successfully")
        elif line.startswith("ERR|"):
            parts = line.split("|", 2)
            if len(parts) >= 3:
                parsed[parts[1].strip()] = (False, parts[2].strip())

    # Build result list
    results = []
    now = datetime.now().isoformat()
    for idx, r in enumerate(records):
        ok, msg = parsed.get(str(idx), (False, "No response from DNS server"))
        target_ip = r["dr_ip"] if direction == "DR" else r["ho_ip"]
        from_ip   = r["ho_ip"] if direction == "DR" else r["dr_ip"]
        results.append({
            "app_name":    r["app_name"],
...
def run_bulk_job(direction, records_to_change):
    """
    Batch DNS bulk change — groups records into batches, runs batches in parallel.
    Each batch = one WinRM session with N Set-DnsServerResourceRecord calls.
    This avoids the 'TIMEOUT after 30s' failure caused by 20+ simultaneous WinRM sessions.
    """
    session_results = []

    # Split into batches
    batches = [records_to_change[i:i+BATCH_SIZE]
               for i in range(0, len(records_to_change), BATCH_SIZE)]
    ...
    with ThreadPoolExecutor(max_workers=PARALLEL_WORKERS) as executor:
        ...
        for batch in batches:
            with job_lock:
                if not job_state["running"]:
                    break
            future = executor.submit(batch_dns_set_records, batch, direction)
        ...
        for future in as_completed(future_map):
            ...
            try:
                ...
                # Entire batch failed — mark all records in it as failed
                batch_results = []
            ...
            with job_lock:
                job_state["done"] += len(batch_results)
                job_state["results"].extend(batch_results)
...
@app.route("/api/records/repair-ho-ip", methods=["POST"])
def repair_ho_ip():
    """
    Restore missing ho_ip values.
    Strategy: if ho_ip is empty but dr_ip exists, derive ho_ip by
    replacing the DR subnet (253) with the HO subnet (252).
    Safe — only touches records where ho_ip is blank/missing.
    """
    ...
    for rec in records:
        ...
        dr = str(rec.get("dr_ip", "")).strip()
        if not ho and dr:
            # Replace .253. with .252. in DR ip to derive HO ip
            derived = dr.replace(".253.", ".252.")
            if derived != dr:
                rec["ho_ip"] = derived
                fixed += 1
    if fixed:
        save_records(records)
    return jsonify({"fixed": fixed, "message": f"Restored ho_ip for {fixed} records"})
...
def set_speed():
    ...
    PARALLEL_WORKERS = max(1, min(int(w), 10))
...
def verify_records():
    """
    Pre-flight check:
    - DR button: compare stored HO IP vs live DNS IP
    - HO button: compare stored DR IP vs live DNS IP
    - Also verifies app_name exists in record
    All 45 records in ONE WinRM batch call — fast.
    """
    data       = request.json or {}
    ...
    # Step 1: local field validation — instant, no DNS
    invalid_ids = set()
    pre_results = {}
    for rec in targets:
    ...
    # Step 2: ONE batch DNS query for all valid records
    valid_targets = [r for r in targets if str(r.get("id")) not in invalid_ids]
    ip_map = {}
    if valid_targets:
        # Split only if more than BATCH_SIZE — for 45 records this is ONE call
        batches = [valid_targets[i:i+BATCH_SIZE]
                   for i in range(0, len(valid_targets), BATCH_SIZE)]
        with ThreadPoolExecutor(max_workers=min(len(batches), PARALLEL_WORKERS)) as executor:
            future_map = {executor.submit(batch_dns_get_records, b): b for b in batches}
            for future in as_completed(future_map):
                try:
                    ip_map.update(future.result(timeout=120))
    ...
    # Step 3: Compare stored IP vs live DNS IP
    # DR  → compare stored HO IP  vs live DNS
    # HO  → compare stored DR IP  vs live DNS
    dns_results = {}
    for rec in valid_targets:
        rid       = str(rec.get("id"))
        stored_ip = rec.get("ho_ip") if direction == "DR" else rec.get("dr_ip")
        target_ip = rec.get("dr_ip") if direction == "DR" else rec.get("ho_ip")
        live_ip, err = ip_map.get(rid, (None, "No result"))
        issues = []

        if err and err != "No result":
            not_found = "NOT_FOUND:" in str(err)
            status    = "not_found" if not_found else "error"
            issues.append(err.replace("NOT_FOUND:", ""))
            can_change = False
        elif live_ip is None:
            # Could not read DNS — allow based on stored values
            status     = "warning"
            issues.append("DNS read failed — will proceed using stored IP values")
            can_change = bool(target_ip)
        elif live_ip == target_ip:
            # Already pointing to target
            status     = "already_done"
            issues.append(f"Already at target IP {target_ip}")
            can_change = False
        elif live_ip == stored_ip:
            # Live DNS matches stored source IP — perfect, ready
            status     = "ready"
            can_change = True
        else:
            # Live DNS has unexpected IP — still allow but warn
            status     = "warning"
            label      = "HO" if direction == "DR" else "DR"
            issues.append(
                f"Live DNS IP {live_ip} ≠ stored {label} IP {stored_ip}"
            )
            can_change = True
    ...
    summary = {
        ...
        "can_change":   sum(1 for r in all_results if r.get("can_change")),
    }
    return jsonify({"records": all_results, "summary": summary})
