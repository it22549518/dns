...
def verify_records():
    ...
    for rec in valid_targets:
        ...
        if err:
            ...
            # DNS fetch returned no result and no error — silent failure
            # If both IPs are stored, allow the change with a warning
            if target_ip and source_ip:
                status     = "warning"
                issues.append("Could not read current DNS IP — proceeding based on stored values")
                can_change = True
            else:
                status     = "error"
                issues.append("Could not read current DNS IP")
                can_change = False
            ...
            status     = "ready"
            can_change = True
        elif direction == "DR" and current_ip == rec.get("dr_ip"):
            # Already at DR
            status     = "already_done"
            issues.append(f"Already pointing to DR IP {current_ip}")
            can_change = False
        elif direction == "HO" and current_ip == rec.get("dr_ip"):
            # Rolling back to HO and currently at DR — normal state, allow it
            status     = "ready"
            can_change = True
        else:
            # Unexpected IP — warn but allow
            status     = "warning"
            issues.append(
                f"Current IP {current_ip} doesn't match expected "
