...
def batch_dns_get_records(records):
    """
    Fetch current IPs for a BATCH of records in ONE PowerShell call.
    Much faster than one call per record — 25 records in ~5s vs ~75s.
    records: list of {"zone":..., "record_name":..., "id":...}
    Returns: dict of id -> (ip, error)
    """
    if not records:
        return {}

    # Build a PowerShell array of zone+name pairs
    pairs = ""
    for r in records:
        pairs += f'    @{{zone="{r["zone"]}";name="{r["record_name"]}";id="{r["id"]}"}},\n'
...
$results = Invoke-Command -ComputerName '{DNS_SERVER}' -Credential $cred -ScriptBlock {{
    param($pairs)
    Import-Module DnsServer -ErrorAction SilentlyContinue
    foreach ($p in $pairs) {{
        try {{
            $rec = Get-DnsServerResourceRecord -ZoneName $p.zone -Name $p.name -RRType A -ErrorAction Stop
            $ip  = ($rec | Select-Object -First 1).RecordData.IPv4Address.ToString()
            Write-Output "OK|$($p.id)|$ip"
        }} catch {{
            Write-Output "ERR|$($p.id)|$($_.Exception.Message)"
        }}
    }}
}} -ArgumentList (,$pairs)
Write-Output $results
"""
    out, _ = run_ps(ps, timeout=max(30, len(records) * 3))
    results = {}
    for line in out.splitlines():
        ...
        if line.startswith("OK|"):
            parts = line.split("|", 2)
            if len(parts) == 3:
                results[parts[1]] = (parts[2].strip(), None)
            ...
            if len(parts) == 3:
                ...
                results[parts[1]] = (None, ("NOT_FOUND:" if not_found else "") + err)
    # Fill in any that got no response
    for r in records:
        rid = str(r["id"])
