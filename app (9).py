...
def verify_records():
    ...
    for rec in valid_targets:
        ...
        if err:
            ...
            # ── Key fix ──────────────────────────────────────────────────────
            # If current_ip doesn't match stored source_ip, auto-correct the
            # stored HO/DR ip to match what DNS actually has, then re-match.
            if current_ip != source_ip:
                if direction == "DR":
                    # Live DNS doesn't match stored ho_ip — update ho_ip to current
                    rec["ho_ip"] = current_ip
                    source_ip    = current_ip
                    records_to_update.append(rec)
                    issues.append(
                        f"HO IP updated from stored value to live DNS IP {current_ip}"
                    )
                else:
                    # Switching back to HO — update dr_ip to current
                    rec["dr_ip"] = current_ip
                    source_ip    = current_ip
                    records_to_update.append(rec)
                    issues.append(
                        f"DR IP updated from stored value to live DNS IP {current_ip}"
                    )
            status     = "ready"
            can_change = True
